...
def transform_data():
    ...
    df = s3.load_file('atp_raw_data.pkl', '/Data')

    df.drop_duplicates()

    df.drop(columns=['tourney_id', 'tourney_name', 'tourney_level', 'match_num', 'winner_entry', 'winner_seed',
                     'winner_ioc',
                     'loser_ioc', 'loser_seed', 'loser_entry', 'score', 'winner_rank', 'l_bpFaced', 'w_bpFaced',
                     'loser_rank',
                     'tourney_date', 'winner_name', 'loser_name'], inplace=True)

    df.dropna(
        subset=['w_ace', 'w_df', 'loser_hand', 'winner_hand', 'winner_rank_points', 'loser_rank_points', 'loser_age'],
        inplace=True)
    ...
    # df.dropna(subset=[ 'loser_hand', 'winner_hand', 'winner_rank_points', 'loser_rank_points', 'loser_age'], inplace=True)
    ...
    s3.write_file(df,'atp_clean_data.pkl', '/Data')
